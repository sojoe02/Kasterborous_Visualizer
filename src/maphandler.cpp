...
bool MapHandler::parseData(const char* filename){
	...
	if(file.is_open()){	
		...
		sprintf(buffer, "\nHeight [m]\t:\t%f\n", dataInfo.areaY);
		ui->printmsg(buffer);
		sprintf(buffer, "Width [m]\t:\t%f\n", dataInfo.areaX);
		ui->printmsg(buffer);
		sprintf(buffer, "Auton Amount\t:\t%d\n", dataInfo.numberOfAutons);
		ui->printmsg(buffer);
		sprintf(buffer, "Event Amount\t:\t%llu\n", dataInfo.eventAmount);
		ui->printmsg(buffer);
		sprintf(buffer, "Time Resolution\t:\t%f\n", (double)1/dataInfo.timeResolution);
		ui->printmsg(buffer);
		sprintf(buffer, "MacroFactor\t:\t%d\n", dataInfo.macroFactor);
		ui->printmsg(buffer);
...
int MapHandler::binData(int timeStep){
	...
	int steps = int(dataInfo.tmuAmount / (dataInfo.timeResolution * double(timeStep))) + 1;
	...
	for(dataItr = dataEvents.begin(); dataItr != dataEvents.end(); dataItr++){
		...
		int seconds = devent.activationTime / dataInfo.timeResolution;
		...
		//ui->printmsg(buffer);		
		intensityMaps.at(seconds/timeStep)->binEvent(devent);
	}
